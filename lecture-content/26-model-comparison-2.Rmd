...
## Recall from Lecture 14: the partial *F* test

Lecture 14 introduced nested-model comparisons and the partial *F* test. We now use that machinery for the factor and covariate models from Lecture 25, where one scientific effect can correspond to several coefficients.
...
The partial *F* tests now connect the geometry to the scientific question:
...
Lecture 23 showed that in an orthogonal design, changing the order of terms does not change the sums of squares attributed to those terms. The Samara data are not orthogonal. Tree and Load are not providing independent pieces of information: the trees were sampled over different Load distributions.
...
Consequently, the variation attributed to Tree depends on whether Load has already been accounted for. These two uses of `anova()` therefore answer different questions.
