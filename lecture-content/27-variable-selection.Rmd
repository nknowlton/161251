...
The richer model has higher adjusted $R^2$, lower residual standard error, and lower AIC in this sample. Adding terms usually improves in-sample fit, so these measures do not establish that the richer model will predict future bike demand better. The real question is whether all those extra variables are worth their degrees of freedom, or whether a smaller subset would predict almost as well.

The point here is not that the richer model must be correct. Realistic datasets often give us many plausible predictors, and manual comparison becomes cumbersome.

With six candidate predictors, manual comparison is manageable. With 10, 20 or 50 plausible predictors, the number of possible models grows rapidly. We now have two possible responses:

1. automate the process of adding and removing terms;
2. avoid discrete include-or-exclude decisions and instead shrink coefficients towards zero.

These are the two approaches developed in the next lecture: stepwise search and penalised regression.
