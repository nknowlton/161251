...
opts_chunk$set(fig.height = 6, fig.width = 9, fig.path = "Figures/", fig.alt = "unlabelled")
...
path_candidates <- c(
  file.path("..","data","fat.csv"),  # knit from lectures/
  file.path("data","fat.csv")        # knit from project root
)

fat_path <- path_candidates[file.exists(path_candidates)][1]

if (is.null(fat_path) || !file.exists(fat_path)) {
  stop("fat.csv not found. Looked in: ", paste(path_candidates, collapse = "; "))
}

Fat <- readr::read_csv(fat_path,show_col_types = FALSE)
message("Loaded Fat from: ", fat_path)
