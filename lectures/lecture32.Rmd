---
title: "Lecture 32: Models with Autoregressive Errors"
subtitle: 161.251 Regression Modelling
author: "Presented by Jonathan Marshall"  
date: "Week 13 of Semester 1, `r lubridate::year(lubridate::now())`"
output:
  slidy_presentation: 
...
```{r templateLink, echo=FALSE, eval=TRUE}
xfun::embed_file("Lecture32.Rmd", name="Lecture32.Rmd", text ="Download the  R markdown file for this lecture.")
```
...
We saw previously that when linear models are fitted to time series data, the residuals can display autocorrelation. We also saw that autocorrelation 
has consequences, in particular
...
```{r whiteNoise, fig.width=8, fig.height=8}
Z1 = rnorm(1000)
...
The `X1` series has correlation of `r round(cor(X1[-1],X1[-999]),3)` at lag1, while the `X2` series has autocorrelation of `r round(cor(X2[-1],X2[-999]),3)` at lag1
...
`r xfun::embed_file(here::here("data/motel.csv"))`


```{r getTourismData, echo=-1, eval=-2}
Tourism <- read.csv(file=here::here("data/motel.csv"), header=TRUE)
...
$$\begin{aligned}
Y_{t} &=& \beta_0 + \beta_1 \mbox{Yr} + \beta_2 z_{t,2} + \beta_3 z_{t,3} 
+ \ldots + \beta_{12} z_{t,12} + \varepsilon_t \\
\varepsilon_t &=& \phi \varepsilon_{t-1} + \eta_{t}\end{aligned}$$ 

where:

- *Y~t~* is the *t*^th^ observation of `RoomNights`;
- *z~t,j~ = 1* if observation *t* is taken in month *j*;
- *z~t,j~ = 0* otherwise.
...
The details are beyond the scope of this paper, but you will meet
    maximum likelihood estimation if you take 161.304.
...
In R, linear models can be fitted using this technique using the
    `gls()` command found in the `nlme` add-on package. Call this package using `library(nlme)` before using the `gls()` command.


    

```{r getLib, include=FALSE}
library(nlme)
```
...
```{r Tourism.gls, echo=-1, eval=-2}
Tourism <- read.csv(file=here::here("data/motel.csv"))
Tourism <- read.csv(file="motel.csv")
Tourism$Yr = Tourism$Year -1979 + Tourism$Month/12
Tourism$Month <- factor(Tourism$Month)
...
Error variance and correlation estimated by REML (restricted
    maximum likelihood).

Estimated value of AR1 correlation parameter is
    $\hat \phi = 0.393$.

Both `Yr` and `Month` are associated with
...
Comparison of the output from `summary(Tourism.gls)` with
    ...
    larger here, particularly for the intercept and the coefficient of    `Yr`.

The only remaining question is, was the use of `gls()` successful?

```{r getResids}
acf(residuals(Tourism.gls, type="n"))
```
