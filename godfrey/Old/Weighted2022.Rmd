...
This follows the idea that Weight represents the  amount of information carried by each datapoint. 
...
Note that the line is overshooting the cluster of points on the left.  The residual plots below show curvature and heteroscedasticity.  The fourth graph shows a simple linear model is highly influenced by the points at the right, which have very large residuals.  
...
plot(weighted.residuals(Sup.wls) ~ X, data=Supervisors)
```
...
The standard deviations are much higher for the highly-educated groups, so that counteracts some of the effect of higher sample sizes $N$. 
...
```{r sds calc}
sds=rep(0,6);  bdrm=1:6
for(j in bdrm ){ sds[j] =sd(rent[bedrooms==j])}
plot(bdrm, sds)
abline(coef(lm(sds~bdrm) ))
```

Let us suppose
$$sd(\mbox{residuals}) \propto Y$$
Then 
...
rent.lm= lm( rent~ bedrooms)
coef0 =coef(rent.lm)
wt1 = 1/predict(rent.lm)

rent.wls1 = lm(rent ~ bedrooms, weights= wt1)
coef1 = coef(rent.wls1)
wt2 = 1/predict(rent.wls1)^2

rent.wls2 = lm(rent ~ bedrooms, weights= wt2)
coef2 = coef(rent.wls2)
wt3 = 1/predict(rent.wls2)^2

rent.wls3 = lm(rent ~ bedrooms, weights= wt3)
coef3 = coef(rent.wls3)
wt4 = 1/predict(rent.wls3)^2

rent.wls4 = lm(rent ~ bedrooms, weights= wt4)
coef4 = coef(rent.wls4)
wt5 = 1/predict(rent.wls4)^2

rent.wls5 = lm(rent ~ bedrooms, weights= wt5)
coef5 = coef(rent.wls5)

cbind(coef0,coef1,coef2,coef3,coef4,coef5)

plot(rent~ bedrooms)
abline ( coef(rent.lm)  , lty=2)
abline(coef(rent.wls4)  , lty=1, col=2)
```
