...
```{r getTourismData, echo=-1, eval=-2}
...
- ***Secular trend*** (or just ***trend***):  tendency of the series to increase or decrease over a long
        period of time.
- ***Seasonal variation*** (or ***seasonality***): describes fluctuations that
        recur in a regular pattern, for example during specific parts of the year (e.g. quarterly or  monthly data).
...
- An additional ***cyclical*** source of variation with irregular-length period (corresponding to business cycles, for example) is sometimes identified.

For simplicity, variation due to day of the week (hebdomadal variation), or time of the day (diurnal variation) is sometimes referred to as seasonality. 

## Modeling a Time Series

As seen before, time series data can be modeled using linear models (although there
    are a number of alternative approaches).

- Long term trend can be modeled using polynomial regression.
- Seasonal effects can be represented by specifying the seasons (e.g. months, quarters, days of the week) as a factor in the model, or using sinusoidal curves. 
...
We have assumed a linear secular trend. Would a quadratic be better?
...
This type of correlation in the sequence of residuals is usually     called **autocorrelation**. As seen before, we can test for autocorrelation using 
...
Time series methods generally relate to the modeling of autocorrelation once trend and the effect of other variables has already been removed, for example looking at the residuals.  
...
To extract the actual numbers from an acf, suppress the plot ask for the $acf part of the acf() output. 

```{r}
acf(Tourism.lm1$residuals, plot=FALSE) 
```

So the first-order autocorrelation is

```{r}
acf(Tourism.lm1$residuals, plot=FALSE)$acf[2]
```
