...
![Outlier county in Florida](../graphics/palmbeach.jpg)
...
- Inspection of a *fitted line plot*  (where fitted
        regression line is superimposed on scatterplot).
...
Approaches in this course are to set the offending datapoint aside (delete it), or use a transformation to reduce its impact (see later), or use weighted regression to reduce its impact (see later).

Alternatively the field of *Robust Statistics* is concerned with more     sophisticated ways of dealing with outliers.
...
The potential influence of a data point can be measured by its
    leverage. For the *i*^th^ data point in a simple linear
...
- data points with extreme *x*-values (i.e. far from $\bar{x}$)
        have greatest leverage.
...
(ii) the model built on data with the data point removed.
 ...
 Cook's distance is a measure of influence that is based
    on exactly this kind of approach.

Cook's distance depends on both the *x* and *y*-values for a data point,
    and will be large for data points with both high leverage and large
...
For example, it may be the variable `climb` has an important effect
    on race time, and should be included in the model.
...
### A Solution
The following code picks out the row number for the maximum residual, and then recomputes the linear model with that removed. 
```{r Hills2 simple}
n=length(Hills$time)
rowofbiggest = (1:n)[Hills.lm$residuals ==max(Hills.lm$residuals)]
Hills2.lm = lm(time~dist, Hills[-rowofbiggest])
```

### A more elegant solution  (for R enthusiasts)
```{r Hills2.lm}
...
# Below: make a row names variable so we can refer to them explicitly 
# filter out the one we don't want
# then go back to having rownames so that our graphs have labelled points
Hills |> rownames_to_column() |>   
filter(rowname != "Bens of Jura") |>   
column_to_rownames() -> Hills2    
...
Suppose we were to delete the $i^{th}$ data point and recalculate the line with it gone.  $$ \hat y_{  -i} = \hat\beta_{0,-i} + \hat\beta_{1,-i} \,\,   x   $$
...
The R function  rstudent()   calculates these.   
