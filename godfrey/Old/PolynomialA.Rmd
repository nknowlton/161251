...
head(Indy ) ; attach(Indy); 
plot(Speed~ Year)
Indy.lm1 = lm(Speed ~ Year)
Indy.lm2 = lm(Speed ~ Year+ I(Year^2) )
Indy.lm3 = lm(Speed ~ Year+ I(Year^2) + I(Year^3))
Indy.lm4 = lm(Speed ~ Year+ I(Year^2) +I(Year^3) + I(Year^4))
...
```{r correlated powers}
Year2= Year^2; Year3=Year^3; Year4=Year^4
summary(cbind( Year, Year2, Year3, Year4))
pairs( cbind( Year, Year2, Year3, Year4))
cor( cbind( Year, Year2, Year3, Year4))
```

The consequence is that the design matrix $X$   has columns that are very  highly correlated  so that the matrix $(X^TX)^{-1}$ which is part of the Least Squares solution
 is numerically unstable -  i.e. small differences, of the size of rounding error, can lead to big differences in the results.  
...
```{r using poly}
Indy.lmp3 = lm( Speed ~ poly( Year, 3, raw=TRUE))
summary(Indy.lmp3)
summary(Indy.lm3)
```
...
Indy.lmp4 = lm( Speed ~ poly( Year, 4, raw=TRUE))
summary(Indy.lmp4)
```
...
Indy.lmo4 = lm(Speed ~ poly(Year,4) )
summary(Indy.lmo4)
Indy.lmo3 = lm(Speed ~ poly(Year,3) )
summary(Indy.lmo3)
```
...
```{r centred} 
Y = Year-1941
Indy.lmc4 = lm(Speed ~ Y + I(Y^2) + I(Y^3) + I(Y^4))
summary(Indy.lmc4)
Indy.lmc3 = lm(Speed ~ Y + I(Y^2) + I(Y^3)  )
summary(Indy.lmc3)
