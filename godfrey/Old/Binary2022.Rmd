...
In the lecture about piecewise regression models, we have seen an example where we construct a variable that indicates whether the predictive variable is below a certain value or not. This is an example of a binary predictor. We will see now examples of how to construct and use them.

Let's go back to the Climate data example, with the two binary predictors `NorthIsland` and `Sea`. In this lecture we will focus on the predictor `NorthIsland` only.
