...
```{r extraPkg, include=FALSE}
library(MPV)
```
<!--- MPV needed for PRESS() --->
...
Less often, however, a statistician will simply want to know what is the best group of explanatory variables for describing the response.
...
(2)	The variances of predictions from the model  at any point $x_0$  are reduced.  
...
#### Scenario 1.  Suppose $\beta_0=50$,  $\beta_1=10$ and $\beta_2=0$.
...
```{r overfitting}
Sim = Sim |> mutate(Y = 50 + 10*tempA + Epsilon)
Sim.lmA = lm( Y ~ tempA, data=Sim)
summary(Sim.lmA)
Sim.lmB = lm( Y ~ tempA + tempB, data=Sim)
summary(Sim.lmB) 
```
...
The major effect is to ***increase the standard errors*** of the estimates – in this case a threefold increase. 



#### Scenario 2. Suppose $\beta_0=50$, $\beta_1=5$ and $\beta_2=5$.
...
```{r underfitting}
Sim = Sim |> mutate( Y = 50 + 5*tempA +5 *tempB + Epsilon)
Sim.lmA = lm( Y ~ tempA, data=Sim)
summary(Sim.lmA)
Sim.lmB = lm( Y ~ tempA + tempB, data=Sim )
summary(Sim.lmB) 
...
- If we omit a variable, it may dramatically decrease  the variance of the other parameter estimates. This is  a desirable outcome. 

- But if we make a mistake and omit an important variable, then the other parameter estimates may be biased.  This is an undesirable outcome. 
...
$$ S = \sqrt{ \frac{1}{n-p}\sum{(y_i - \hat y_i)^2} }$$
...
We can calculate the predictive sum of squares for a linear model using the `PRESS()` function from the `MPV` package, but we need to enter a new function to get the $R^2$ for predictions. We'll use  the following code from  https://stackoverflow.com/questions/64224124/how-to-calculate-predicted-r-sq-in-r .
...
summary(Climate.lm1)
summary(Climate.lm1)$sigma
summary(Climate.lm1)$adj.r.squared
pred_r_squared(Climate.lm1)

Climate.lm2 = lm( MnJlyTemp ~ Lat+ Height, data=Climate)
summary(Climate.lm2)
summary(Climate.lm2)$sigma
summary(Climate.lm2)$adj.r.squared
pred_r_squared(Climate.lm2)

Climate.lm3 = lm( MnJlyTemp ~ Lat+ Height+ Sea, data=Climate)
summary(Climate.lm3)
summary(Climate.lm3)$sigma
summary(Climate.lm3)$adj.r.squared
pred_r_squared(Climate.lm3)
```

For these first three predictors,   which have significant *p*-values, the S has gone down each time, and  the adjusted $R^2$ and $R^2_{pred}$ have gone up each time, so there is no reason to doubt that `Lat`, `Height`, and `Sea` should all be included.   Also note the regression coefficient for `Height` altered by about 2 standard errors, which suggests that `Sea` is correcting for bias.   All the more reason for including it. 

```{r fourth variable}
Climate.lm4 = lm( MnJlyTemp ~ Lat+ Height+ Sea + NorthIsland, data=Climate)
summary(Climate.lm4)
summary(Climate.lm4)$sigma
summary(Climate.lm4)$adj.r.squared
pred_r_squared(Climate.lm4)
```

Same conclusions as before.  We should include `NorthIsland`.  Note including it has induced big changes in the coefficients (especially `Lat`) which again is evidence it is important. 

```{r fifth variable}
Climate.lm5 = lm( MnJlyTemp ~ Lat+ Height+ Sea + NorthIsland+ Rain, data=Climate)
summary(Climate.lm5)
summary(Climate.lm5)$sigma
summary(Climate.lm5)$adj.r.squared
pred_r_squared(Climate.lm5)
```

This time the `Rain` variable is not significant (P=0.1).   However the S has gone down, and  the adjusted R-squared and pred_r_squared have gone up.  So we may decide that if our aim is to predict for new data, then we should include this variable in the model. 

```{r sixth variable}
Climate.lm6 = lm( MnJlyTemp ~ Lat+ Height+ Sea + NorthIsland+ Rain + Long, data=Climate)
summary(Climate.lm6)
summary(Climate.lm6)$sigma
summary(Climate.lm6)$adj.r.squared
pred_r_squared(Climate.lm6)
```

At this point all the indications are bad:  The *P*-value for `Long` is  above 0.5, the adjusted R-squared has gone down, the S has gone up and the pred_r_squared has gone down.  

In conclusion we should include the first four variables, and optionally the fifth one, `Rain`. 

The following lists all the statistics for comparison purposes. 


<!--- fix the following --->

```{r} 
sigmas = c(summary(Climate.lm1)$sigma,summary(Climate.lm2)$sigma,summary(Climate.lm3)$sigma,summary(Climate.lm4)$sigma,summary(Climate.lm5)$sigma,summary(Climate.lm6)$sigma )
adjR2s = c(summary(Climate.lm1)$adj.r.squared,summary(Climate.lm2)$adj.r.squared,summary(Climate.lm3)$adj.r.squared,summary(Climate.lm4)$adj.r.squared,summary(Climate.lm5)$adj.r.squared,summary(Climate.lm6)$adj.r.squared)
R2preds = c(pred_r_squared(Climate.lm1),pred_r_squared(Climate.lm2),pred_r_squared(Climate.lm3),pred_r_squared(Climate.lm4),pred_r_squared(Climate.lm5),pred_r_squared(Climate.lm6))
Models = c(1,2,3,4,5,6)
cbind(Models, sigmas, adjR2s, R2preds)
```
