...
As a simple way of spreading them out,  I randomize the order of numbers in column TempB. 
...
$$= \gamma_0(k) +\gamma_1(k) X  + \gamma_2(k) Z +\varepsilon$$
...
```{r read electric, eval=-1, echo=-2}
electric = read.csv("../../data/electric.csv", header=TRUE)
electric;   attach(electric)
cor(electric)
```
...
```{r single variables}
elec.lm1 = lm(Bill ~ Area);  summary(elec.lm1)
elec.lm2= lm(Bill ~ Income);  summary(elec.lm2)
elec.lm3 = lm(Bill ~ Income + Area);summary(elec.lm3)
```
...
```{r pair of variables}
elec.lm4 = lm(Bill ~ Area+ Persons) ; summary(elec.lm4)
elec.lm5= lm(Bill ~ Income+ Persons); summary(elec.lm5)
