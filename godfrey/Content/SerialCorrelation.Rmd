...
## ACF: Visualizing Autocorrelation Structure

The **Autocorrelation Function (ACF)** plot shows the correlation between observations at different time lags. For lag $k$, it computes:

$$r_k = \frac{\sum_{t=k+1}^n (y_t - \bar{y})(y_{t-k} - \bar{y})}{\sum_{t=1}^n (y_t - \bar{y})^2}$$

The ACF plot helps us see:
- **Positive autocorrelation**: ACF decays slowly (exponentially or sinusoidally)
- **Negative autocorrelation**: ACF alternates in sign
- **White noise**: Only lag 0 is significant; all other lags are near zero

```{r acf-plot, fig.width=8, fig.height=4}
# ACF of residuals
acf(residuals(MoneyStock.lm1), main = "ACF of MoneyStock Residuals")
```

The slow decay in the ACF confirms positive autocorrelation — residuals at time $t$ are correlated with residuals several time steps earlier.

## Ljung-Box Test: A General Test for Autocorrelation

The **Ljung-Box test** is a more general test than Durbin-Watson. It tests whether a group of autocorrelations (up to lag $h$) are jointly zero:
...
where $n$ is the sample size and $r_k$ is the autocorrelation at lag $k$. Under $H_0$, $Q^*$ follows a $\chi^2$ distribution with $h - p$ degrees of freedom (where $p$ is the number of parameters estimated in the model).

```{r ljung-box}
# Ljung-Box test on residuals (testing up to lag 10)
Box.test(residuals(MoneyStock.lm1), lag = 10, type = "Ljung-Box")
```

A small p-value indicates that the residuals are not white noise — there is remaining autocorrelation structure that our model has not captured.

## Differencing: Removing Trend and Non-Stationarity

When a time series shows a trend or random walk behaviour, the series is **non-stationary** — its statistical properties (mean, variance) change over time. One way to make it stationary is **differencing**: computing the changes between consecutive observations.

$$y'_t = y_t - y_{t-1}$$

If the original series $y_t$ has a trend, the differenced series $y'_t$ may be stationary (constant mean and variance over time).

```{r differencing-example}
# Original series
plot(MoneyStock$ConsExp, type = "l", main = "Consumer Expenditure Over Time")

# Differenced series (changes)
diff_cons <- diff(MoneyStock$ConsExp)
plot(diff_cons, type = "l", main = "Changes in Consumer Expenditure")
```

The differenced series often looks more like white noise — no obvious trend, and the ACF should drop to zero quickly.

### Random Walk: A Baseline Model

A **random walk** is the simplest model for a non-stationary time series:

$$y_t = y_{t-1} + \varepsilon_t$$

where $\varepsilon_t$ is white noise. This means the best prediction for tomorrow is today's value. The changes $y_t - y_{t-1} = \varepsilon_t$ are unpredictable.

Random walks have:
- No predictable trend (but can drift)
- Variance that increases with time
- ACF that decays very slowly

If differencing a series produces white noise, then the original series was a random walk. This is a useful baseline: if your regression model cannot beat a random walk, it may not be adding value.
