...
## Example:  Hours of sunshine

We consider the Climate data, but this time we will use the variable **Sun** as the response, and try to explain it in terms of the other variables (main effects only).
We will use a backwards elimination approach.  Initially we will do this by hand for illustration purposes. 
...
- The so called information criteria can be used as measures
    of model quality.
...
We have found two different models, Climate.lm6 and Climate.s2.  These not only have  different numbers of variables, but are not even nested!  However we can compare them using the adjusted R-square, or the function AIC().
...
```{r step}
climate.null=lm(Sun~ 1, data=climate)
climate.step <- step(climate.null, scope = ~Lat+Long+MnJanTemp+MnJlyTemp +Height+Rain+Sea+NorthIsland , data=climate,  direction="both")
...
```{r step2}
climate.full=lm(Sun~ Lat+Long+MnJanTemp+MnJlyTemp +Height+Rain+Sea+NorthIsland, data=climate)
...
```{r Sun vs geography}
climate.null=lm(Sun~ 1, data=climate)
climate.geog <- step(climate.null, scope = ~Lat*Long*Height*Sea*NorthIsland , data=climate,  direction="both")
summary(climate.geog)
plot(climate.geog$fitted.values ~ Long, pch=2-Sea ,col=NorthIsland+3, data=climate)
 
```
In the plot, green locations are for the South Island and blue for the North Island, triangles represent inland places and circles are places by the sea.    The plot indicates, that, for coastal towns/cities,  locations on  the eastern side of each island (higher longitude)  tend to have more sun than locations on the western side. But for inland places the longitude makes little difference. 

Next we can try entering the other weather variables.

```{r further stepwise}

climate.geog2 <- step(climate.geog, scope = ~Lat*Long*Height*Sea*NorthIsland*MnJanTemp*MnJlyTemp*Rain , data=climate,  direction="both")
...
```{r weeding out non-significant terms}
