...
- [x] Verify student and solution documents, data references, site links and R-rendered HTML pages.
...
Pandoc parsed all eight canonical lectures and both student/solution pairs for Labs 11 and 12. Chunk names are unique, code fences are balanced, referenced data files exist, and the Lab 12 pair has the same chunk names and order. Independent numerical checks used the bundled CSVs for climate search, electricity harmonic comparison, CPS5 grouped counts, Seoul consecutive dates and Fourier comparison, Perth coefficients, and the 180 monthly tourism and 45 quarterly Perth dates. `git diff --check` passed. Rscript and Quarto are unavailable locally; the R-enabled GitHub Actions build supplies execution and rendering checks.

The first R-enabled build rendered the book and all Slidy lectures. It stopped while knitting the new Lab 12 because the `gls` tidying method was not registered in that document; `broom.mixed`, already used by Lecture 35 and installed in the workflow, is now loaded in both Lab 12 versions. The [follow-up build](https://github.com/nknowlton/161251/actions/runs/36266814967) passed course validation, bookdown, all Slidy lectures, both Lab 12 HTML knits, the final site artefact checks and Pages deployment. No local visual inspection of the rendered pages was possible; the successful render and published-file check establish that the output exists and executes, not that every figure has been visually reviewed.
