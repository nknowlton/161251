...
The first R-enabled build rendered the book and all Slidy lectures. It stopped while knitting the new Lab 12 because the `gls` tidying method was not registered in that document; `broom.mixed`, already used by Lecture 35 and installed in the workflow, is now loaded in both Lab 12 versions. The follow-up build is the check for the complete site and both lab pages.
